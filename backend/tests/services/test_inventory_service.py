...
from app.core.enums import ScannerType, ScanStatus, TargetType
...
def _make_package(scan_id, hosts=None):
    ...
    return AssessmentPackage(
        scan_id=scan_id, scanner_type=ScannerType.NMAP, parsed_hosts=hosts or []
    )
...
def test_new_asset_created(db_session, inventory_service, scan_job):
    ...
    asset = db_session.execute(
        select(Asset).where(Asset.ipv4 == "10.0.0.1")
    ).scalar_one()
...
def test_existing_asset_reused(db_session, inventory_service, scan_job, target):
    ...
    host = ParsedHost(
        ipv4="10.0.0.2", hostname="new-host", operating_system="FreeBSD 14"
    )
    pkg = _make_package(scan_job.id, [host])
    ...
    count = db_session.execute(
        select(func.count(Asset.id)).where(Asset.ipv4 == "10.0.0.2")
    ).scalar()
    assert count == 1

    db_session.expire_all()
    asset = db_session.execute(
        select(Asset).where(Asset.ipv4 == "10.0.0.2")
    ).scalar_one()
...
def test_asset_metadata_not_overwritten_with_empty(
    ...
    db_session.expire_all()
    asset = db_session.execute(
        select(Asset).where(Asset.ipv4 == "10.0.0.3")
    ).scalar_one()
...
def test_service_created(db_session, inventory_service, scan_job):
    svc = ParsedService(
        port=22, protocol="tcp", service_name="ssh", product="OpenSSH", version="8.9"
    )
    ...
    asset = db_session.execute(
        select(Asset).where(Asset.ipv4 == "10.0.1.1")
    ).scalar_one()
    service = db_session.execute(
        select(NetworkService).where(
            NetworkService.asset_id == asset.id, NetworkService.port == 22
        )
    ).scalar_one()
...
def test_service_reused_and_updated(db_session, inventory_service, scan_job, target):
    ...
    parsed_svc = ParsedService(
        port=80, protocol="tcp", service_name="http", product="Apache", version="2.4.58"
    )
...
def test_vulnerability_created(db_session, inventory_service, scan_job):
    vuln = ParsedVulnerability(
        name="CVE-Test", severity_score=7.5, severity_rating="High", cve="CVE-2024-0001"
    )
    svc = ParsedService(
    ...
    v = db_session.execute(
        select(Vulnerability).where(Vulnerability.cve == "CVE-2024-0001")
    ).scalar_one()
...
def test_vulnerability_reused_with_cve(db_session, inventory_service, scan_job, target):
    ...
    count = db_session.execute(
        select(func.count(Vulnerability.id)).where(Vulnerability.cve == "CVE-2024-9999")
    ).scalar()
    assert count == 1
...
def test_vulnerability_duplicate_prevention_null_cve(
    db_session, inventory_service, scan_job, target
):
    existing_v = Vulnerability(
        name="MiscConfig", severity_score=3.0, severity_rating="Low", cve=None
    )
    db_session.add(existing_v)
    db_session.flush()
    ...
    svc = ParsedService(
        port=8080, protocol="tcp", service_name="http-alt", vulnerabilities=[vuln]
    )
    ...
    count = db_session.execute(
        select(func.count(Vulnerability.id)).where(
            Vulnerability.name == "MiscConfig", Vulnerability.cve.is_(None)
        )
    ).scalar()
    assert count == 1
...
def test_scan_finding_created(db_session, inventory_service, scan_job):
    ...
    svc = ParsedService(
        port=22, protocol="tcp", service_name="ssh", vulnerabilities=[vuln]
    )
...
def test_scan_status_completed(db_session, inventory_service, scan_job):
    ...
    updated = db_session.execute(
        select(ScanJob).where(ScanJob.id == scan_job.id)
    ).scalar_one()
...
def test_rollback_on_failure(db_session, inventory_service, scan_job):
    ...
    vuln = ParsedVulnerability(
        name="RollbackTest", severity_score=1.0, severity_rating="Low"
    )
    svc = ParsedService(
        port=80, protocol="tcp", service_name="http", vulnerabilities=[vuln]
    )
    ...
    count = db_session.execute(
        select(func.count(Asset.id)).where(Asset.ipv4 == "10.0.6.1")
    ).scalar()
    assert count == 0
...
def test_scan_status_failed_after_error(db_session, inventory_service, scan_job):
    ...
    with (
        patch.object(
            inventory_service, "_upsert_asset", side_effect=RuntimeError("db error")
        ),
...
def test_cve_normalization(db_session, inventory_service, scan_job):
    ...
    s1 = ParsedService(
        port=443, protocol="tcp", service_name="https", vulnerabilities=[v1]
    )
    ...
    v2 = ParsedVulnerability(
        name="CVE-Test", severity_score=7.0, severity_rating="High", cve="CVE-2024-0002"
    )
    s2 = ParsedService(
        port=443, protocol="tcp", service_name="https", vulnerabilities=[v2]
    )
    ...
    v = db_session.execute(
        select(Vulnerability).where(Vulnerability.name == "CVE-Test")
    ).scalar_one()
    assert v.cve == "CVE-2024-0002"

    count = db_session.execute(
        select(func.count(Vulnerability.id)).where(Vulnerability.name == "CVE-Test")
    ).scalar()
    assert count == 1
...
def test_duplicate_hosts_in_package(db_session, inventory_service, scan_job):
    ...
    asset = db_session.execute(
        select(Asset).where(Asset.ipv4 == "10.0.9.1")
    ).scalar_one()
    assert asset.hostname == "host-second"
    assert asset.operating_system == "Linux Kernel"

    count = db_session.execute(
        select(func.count(Asset.id)).where(Asset.ipv4 == "10.0.9.1")
    ).scalar()
    assert count == 1
...
def test_duplicate_services_in_package(db_session, inventory_service, scan_job):
    ...
    asset = db_session.execute(
        select(Asset).where(Asset.ipv4 == "10.0.10.1")
    ).scalar_one()
    services = (
        db_session.execute(
            select(NetworkService).where(NetworkService.asset_id == asset.id)
...
def test_duplicate_findings_in_package(db_session, inventory_service, scan_job):
    ...
    s = ParsedService(
        port=443, protocol="tcp", service_name="https", vulnerabilities=[v1, v2]
    )
