...
class TestCreateTarget:
    ...
    def test_create_duplicate_target(self, service, mock_repo):
        ...
        mock_repo.get_by_value.return_value = Target(id=uuid.uuid4(), target="192.168.1.1")
...
class TestUpdateTarget:
    def test_update_valid_target(self, service, mock_repo):
        ...
        existing = Target(id=target_id, target="old.com", target_type=TargetType.HOSTNAME)
        
        mock_repo.get_by_id.return_value = existing
        mock_repo.get_by_value.return_value = None
        mock_repo.update.return_value = existing  # Repo typically returns the same updated instance
    ...
    def test_update_same_target_value_is_allowed(self, service, mock_repo):
        ...
        target_id = uuid.uuid4()
        existing = Target(id=target_id, target="192.168.1.1", target_type=TargetType.IPV4)
        ...
        result = service.update_target(target_id, UpdateTargetRequest(target="192.168.1.1"))
