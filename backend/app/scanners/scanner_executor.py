...
class ScannerExecutor:
    ...
    def _validate_executable(self, scanner_type: ScannerType, cmd_args: List[str]) -> str:
        ...
        if basename not in allowed and executable not in allowed:
            ...
            if configured_exec and (executable == configured_exec or os.path.basename(configured_exec).lower() == basename):
                return executable
    ...
    def run_scanner(
        ...
        """
        executable = self._validate_executable(scanner_type, cmd_args)
        ...
        try:
            # shell=False is mandatory to prevent command injection
            ...
            status = ExecutionStatus.ERROR
            raise ScannerExecutionException(f"Scanner process execution failed: {e}") from e
