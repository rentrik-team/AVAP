...
from app.core.enums import ScannerType, ScanProfile, ExecutionStatus
...
class OpenVASAdapter(BaseScannerAdapter):
    ...
    def build_command(self, target: str, scan_profile: ScanProfile, output_path: str) -> List[str]:
        ...
        """
        raise NotImplementedError("OpenVAS does not support direct command line execution.")
    ...
    def execute(self, scan_id: uuid.UUID, target: str, scan_profile: ScanProfile) -> ScanArtifact:
        ...
        if is_stub:
            logger.info("OpenVAS host is localhost. Running in stub/simulation mode.", extra={"scan_id": str(scan_id)})
            ...
            try:
                ...
                raise ScannerExecutionException(f"Failed to write mock OpenVAS output: {e}") from e
